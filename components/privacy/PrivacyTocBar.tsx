...
import type { PrivacySection } from "@/lib/privacy-content";
...
export function PrivacyTocBar({ sections, triggerId }: Props) {
  ...
  // Track when the floating bar should be revealed: once the inline TOC
  // (its document-position bottom) has scrolled above the navbar.
  useEffect(() => {
  ...
  // Pick the topmost intersecting one within the active band.
  useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    const observer = new IntersectionObserver(
      ...
      {
        // Active "band": from below the navbar+toc-bar to mid-screen.
  ...
  // Close dropdown when bar gets hidden (e.g. user scrolled back up past TOC)
  useEffect(() => {
      ...
      <Container>
        <div className="h-12 md:h-14 flex items-center justify-between gap-3">
          <button
            ...
            onClick={() => setOpen((v) => !v)}
            aria-expanded={open}
            aria-controls="privacy-toc-dropdown"
            className={cn(
